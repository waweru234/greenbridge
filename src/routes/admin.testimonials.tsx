...
function TestimonialsAdmin() {
  ...
  async function save(r: Row) {
    ...
    if (id.startsWith("new-")) {
      const { data, error } = await supabase.from("testimonials").insert(patch).select().single();
      setSaving(null);
  ...
  function add() {
    setRows((rs) => [...rs, { id: `new-${Date.now()}`, name: "", role: "", company: "", quote: "", avatar_url: "", sort_order: rs.length + 1, is_active: true }]);
  }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Quotes shown on your website.</p>
        </div>
      ...
      {rows.map((r) => (
        <div key={r.id} className="rounded-2xl bg-card p-5 shadow-soft border border-border space-y-3">
          <div className="grid gap-3 md:grid-cols-12">
            <input className="input md:col-span-4" placeholder="Name" value={r.name} onChange={(e) => setRows((rs) => rs.map((x) => x.id === r.id ? { ...x, name: e.target.value } : x))} />
            <input className="input md:col-span-3" placeholder="Role" value={r.role ?? ""} onChange={(e) => setRows((rs) => rs.map((x) => x.id === r.id ? { ...x, role: e.target.value } : x))} />
            <input className="input md:col-span-3" placeholder="Company" value={r.company ?? ""} onChange={(e) => setRows((rs) => rs.map((x) => x.id === r.id ? { ...x, company: e.target.value } : x))} />
          ...
          <textarea rows={3} className="input" placeholder="Quote" value={r.quote} onChange={(e) => setRows((rs) => rs.map((x) => x.id === r.id ? { ...x, quote: e.target.value } : x))} />
